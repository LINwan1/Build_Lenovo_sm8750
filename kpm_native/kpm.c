...
/* ============================================================================
 *  a13d71f6 ioctl 入口适配层
 *  上游 743ddce9f7 的入口是 prctl 风格 sukisu_handle_kpm(arg3,arg4,arg5)，
 *  a13d71f6 改为 ioctl：sukisu_handle_kpm(control_code,arg1,arg2,result_code)。
 *  这里只重写入口，加载/卸载/列表等核心逻辑沿用 743 版原生实现。
 * ==========================================================================*/

extern const char *kpver;

static long kpm_compat_load(unsigned long arg1, unsigned long arg2)
{
	...
	if (arg2 != 0) {
		if (strncpy_from_user(kernel_args_buffer, (const char __user *)arg2,
				      sizeof(kernel_args_buffer) - 1) < 0)
			return -EFAULT;
	}

	return sukisu_kpm_load_module_path(kernel_load_path, kernel_args_buffer, NULL);
}

static long kpm_compat_unload(unsigned long arg1)
{
	...
	return sukisu_kpm_unload_module(kernel_name_buffer, NULL);
}

static long kpm_compat_info(unsigned long arg1, unsigned long arg2)
{
	char kernel_name_buffer[256] = { 0 };
	...
	return sukisu_kpm_info(kernel_name_buffer, (char __user *)arg2);
}

static long kpm_compat_version(unsigned long arg1, unsigned long arg2)
{
	char buf[64] = { 0 };
	...
	strscpy(buf, kpver ? kpver : "0.10", sizeof(buf));
	len = (int)strlen(buf);
	if (len >= (int)outlen)
		len = (int)outlen - 1;

	if (copy_to_user((void __user *)arg1, buf, len + 1))
		return -EFAULT;
		      ...
		      unsigned long arg2, unsigned long result_code)
{
	...
	switch (control_code) {
	case KSU_KPM_LOAD:
		res = kpm_compat_load(arg1, arg2);
		break;
	case KSU_KPM_UNLOAD:
		res = kpm_compat_unload(arg1);
		break;
	case KSU_KPM_NUM:
		res = sukisu_kpm_num();
		break;
	case KSU_KPM_LIST:
		res = sukisu_kpm_list((char __user *)arg1, (unsigned int)arg2);
		break;
	case KSU_KPM_INFO:
		res = kpm_compat_info(arg1, arg2);
		break;
	case KSU_KPM_CONTROL:
		/* 743 版加载器不含「调用模块内函数」能力，明确返回未实现 */
		pr_info("kpm: KSU_KPM_CONTROL not supported by native loader\n");
		res = -ENOSYS;
		break;
	case KSU_KPM_VERSION:
		res = kpm_compat_version(arg1, arg2);
		break;
	default:
		res = -EINVAL;
		break;
	...
	if (result_code &&
		...
		pr_info("kpm: Copy to result_code failed.\n");
...
{
	return (control_code >= KSU_KPM_LOAD &&
		control_code <= KSU_KPM_VERSION) ? 1 : 0;
}
...
int do_kpm(void __user *arg)
{
	...
	return sukisu_handle_kpm(cmd.control_code, cmd.arg1,
				 cmd.arg2, cmd.result_code);
}
